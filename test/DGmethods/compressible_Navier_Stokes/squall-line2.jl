...
@inline function preflux(Q,VF, aux, _...)
    ...
    e_tot   = ρinv*E
    #e_int   = ρinv*(E - (U^2 + V^2+ W^2)/(2*ρ) - ρ * gravity * z)
    q_tot   = ρinv*QT
    q_liq   = ρinv*QL
    q_rai   = ρinv*QR
    ...
    #    w_rai = terminal_velocity(q_rai, ρ)
    #else
        w_rai = 0 .* Q[_QT]
    #end
    
    #(P, u, v, w, q_tot, q_liq,T,θ)
    (u, v, w, w_rai, ρ, q_tot, q_liq, q_rai, e_tot)
end
...
# max eigenvalue
@inline function wavespeed(n, Q, aux, t, u, v, w, w_rai, ρ, q_tot, q_liq, q_rai, e_tot)

    @inbounds begin
        (n[1] * abs(u) + n[2] * abs(v) + n[3] * max(abs(w), abs(w_rai), abs(w - w_rai))) +
